    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (response.data.items && Array.isArray(response.data.items)) {
          ...
          const hasInvalidItems = response.data.items.some(
            (item) => !item.productId || !item.productId._id
          );

          if (hasInvalidItems) {
            console.warn(
              "Cart contains invalid items with missing productId",
            ...
            if (response.data.items.length === 0) {
              toast.error(
                "Your cart contains invalid products. Please try adding products again."
              );
            } else {
        ...
        if (response.data.items && Array.isArray(response.data.items)) {
          const allProductIds = response.data.items.map(
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (selectedItems.length !== selectedProductIds.length) {
        ...
        toast.error(
          "Selection mismatch detected. Please refresh the page and try again."
        );
        return;
      }
      ...
      try {
        ...
        const response = await paymentService.createPaymentSelected(
          checkoutPayload
        );
        toast.dismiss(loadingToast);
    ...
    try {
      ...
      if (vnp_ResponseCode === "00") {
        ...
        if (selectedProductIds.length > 0) {
          ...
          try {
            // Check if searchParams exists before passing to the service
            if (!searchParams) {
              throw new Error("Missing payment parameters");
            }

            // First verify payment with VNPay using verify-selected API
            const verifyResult = await paymentService.verifySelectedPayment(
              searchParams
            );
            ...
            <div className="lg:col-span-8">
              <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                ...
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="flex gap-4 mb-6 last:mb-0 pb-6 last:pb-0 border-b last:border-0"
                  >
      ...
      <div className="container mx-auto p-4 max-w-7xl">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2 mt-2">
            <Badge variant="secondary">{cart.items.length} products</Badge>
            <Badge
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (selectedProductIds.length === cart.items.length) {
                          ...
                          // Select all
                          setSelectedProductIds(
                            cart.items.map((item) => item.productId._id)
                          );
                        }
                      }}
                        ...
                        {item && item.productId ? (
                          <div className="flex gap-4">
                            ...
                            <div className="relative h-24 w-24 flex-shrink-0 overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
                              <Image
                                src={
                                  Array.isArray(item.productId.productImages) &&
              ...
              <CardContent className="pb-4 space-y-4">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">
                    Subtotal ({selectedProductIds.length} items)
                  </span>
