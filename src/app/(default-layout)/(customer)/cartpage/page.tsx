...
export default function CartPage() {
  ...
  const handleUpdateQuantity = async (
    ...
    try {
      ...
      if (
        newQuantity >= 1 &&
        newQuantity <= (existingItem.productId.stock || 0)
      ) {
        await cartService.addToCart(productId, quantityChange);
        await fetchCart();
      } else {
        toast.error(
          `Quantity must be between 1 and ${existingItem.productId.stock || 0}`
        );
      }
    } catch (error) {
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (selectedItems.length === 0) {
        ...
        toast.error(
          "Your cart contains invalid products. Please try refreshing or contact support."
        );
        return;
      }
      ...
      if (selectedItems.length < cart.items.length) {
        toast(
          ...
          } invalid item(s) were removed from checkout`,
          {
            icon: "⚠️",
            style: {
              background: "#FFF3CD",
      ...
      if (error instanceof TypeError && error.message.includes("_id")) {
        toast.error(
          "There was a problem with some products in your cart. Please try refreshing the page."
        );
      } else {
      ...
      <div className="container mx-auto p-4 max-w-7xl">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2 mt-2">
            <Badge variant="secondary">{cart.items.length} sản phẩm</Badge>
          </div>
        </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Đơn hàng sẽ được giao trong vòng 2-3 ngày làm việc kể từ
